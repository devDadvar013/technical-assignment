...
import {
  useJourneyScroll,
  useJourneyStepScroll,
  usePhoneScreenScroll,
} from '@/hooks/useJourneyScroll';
import { Container, SectionHeader } from '@/components/ui/primitives';

function JourneyStepItem({ step }: { step: JourneyStepType }) {
  const ref = useRef<HTMLDivElement>(null);
  const { opacity, x } = useJourneyStepScroll(ref);

  return (
    <motion.div ref={ref} className="flex items-start gap-6 pl-2 max-sm:flex-col max-sm:gap-3" style={{ opacity, x }}>
      <div className="relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 border-accent bg-surface font-display text-xs font-bold text-accent-light">
        {step.number}
      </div>
      <div>
        <h3 className="mb-2 font-display text-xl font-semibold tracking-tight">{step.title}</h3>
        <p className="mb-3 text-sm leading-relaxed text-text-muted">{step.description}</p>
...
function PhoneScreenItem({ screen, index, scrollYProgress }: { screen: PhoneScreen; index: number; scrollYProgress: ReturnType<typeof useJourneyScroll>['scrollYProgress'] }) {
          ...
          <div className="relative">
            ...
            <div className="flex flex-col gap-12 max-sm:gap-8">
              {journeySteps.map((step) => (
                <JourneyStepItem key={step.number} step={step} />
              ))}
            </div>
          </div>
